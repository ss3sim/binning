...
d <- ddply(d, .(data,variable, tvalue), summarize,
           ...
           count = length(value))
d$tvalue <- as.numeric(as.factor(d$tvalue))
...
for(i in seq_along(mydata)){
    for(j in seq_along(myvariables)){
        ...
        with(d.temp1, {
            points(x=tvalue, y=median_, pch=16, cex=.5, col=1)
            segments(x0=tvalue, y0=l2, y1=u2, col=1)
            text(x=tvalue, y=1.1,
                 labels=mare, col=1, adj=c(0,0), cex=.7)})
        if(nrow(d.temp2)>0){
            with(d.temp2, {
                points(tvalue, y=rep(0, NROW(d.temp2)), pch="x",
                       col=1, cex=.7)
                text(x=tvalue, y=.8+ifelse(1=='black',0, .3),
