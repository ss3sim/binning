...
for(i in seq_along(mydata)){
    for(j in seq_along(myvariables)){
        ...
        if(j==1){
            ...
            axis(2, at=c(-.5,0,.5), col=col.tick, mgp=par()$mgp, tck=par()$tck)
        }
        box(col=col.border)
    }
}
mtext("Data Rich", side=2, line=.9, cex=.6, outer=TRUE, at=.75)
mtext("Data Poor", side=2, line=.9, cex=.6, outer=TRUE, at=.25)
mtext("Tail Compression Value", side=1, line=1.85, cex=par()$cex.lab, outer=TRUE)
