...
####Lastupdate : Adapted by Cole for growth workshop. Fixed the loop
####through the species cases, didn't work for me. Also the FMSY
####calculations weren't automated between species
####Purpose    :
####Packages   :
####Inputs     :
####Outputs    :
...
for(spp in seq_along(spp.case)) {
    ...
    f.info <- c(paste0("years; c(", paste(
        ...
        collapse = ","), ")"),
                paste0("years_alter; c(", paste(
                    ...
                    start:end,
                    collapse = ","), ")"))
    ## Cole updated this, should be checked. 11/3/14
    fmsy <- ifelse(spp.case[spp]=="fla", 0.314, .07)
    f0 <- c(f.info, paste0("fvals; c(", paste(
        ## F vals for each year
        c(rep(0, start.fishery - start), rep(fmsy, end - start.fishery + 1)
          ), collapse = ","), ")"))
    ...
    f1 <- c(f.info, paste0("fvals; c(", paste(
        ## F vals for each year
        fhigh <-
        c(rep(0, start.fishery - start - 1),
          seq(0, 2.5*fmsy, length.out = 41),
          seq(2.5*fmsy, fmsy, length.out = 35)),
        collapse = ","),")"))
    ...
    allyears <- paste0("list(c(", paste(c(
        ...
        seq(start.fishery + 46, end)),
                                        collapse = ","), "), c(", paste( c(
                                            ## Survey
                                            seq(start.survey, end, by = 2)), collapse = ","), "))")
    fishyears <- paste0("list(c(", paste(c(
        ## Fishery
        seq(start.fishery, start.fishery + 10, by = 10),
        seq(start.fishery + 20, start.fishery + 45, by = 5),
        seq(start.fishery + 46, end)), collapse = ","), "))")
    ...
    writeS <- function(vals, case) {
        ...
        trash <- lapply(seq_along(parnames), function(x) {
...
##          I = "index", L = "lcomp", R = "R")
## scen <-
##     expand_scenarios(list(A = 0, L = 0, X = c(0), E = 0,
##                           F = c(0), I = c(0), R = c(0)), species = "fla")
## writeL <- function(fleets, Nsamp, years, case, lengthbin_vector=NULL) {
## 	l <- c(paste("fleets;", fleets),
## 	       paste("Nsamp;", Nsamp),
## 	       paste("years;", years),
## 	       "cpar; 1",
## 	       ifelse(!is.null(lengthbin_vector),
##                       paste("lengthbin_vector; ",lengthbin_vector),
##                       paste("lengthbin_vector; NULL")),
## 	       "write_file; TRUE")
## 	writeLines(l, paste0("conference cases/lcomp", case, "-fla.txt"))
## }
## ## This is how Kelli originall called them, I'm adding bin vector below
## writeL(fleets = bothfleets, Nsamp = allsamples, years = allyears, case = 0)
## writeL(fleets = justfish, Nsamp = fishsamples, years = fishyears, case = 1)
## writeA(fleets = bothfleets, Nsamp = allsamples, years = allyears, case = 0)
## writeA(fleets = "NULL", Nsamp = "NULL", years = "NULL", case = 1)
