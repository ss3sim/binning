...
layout(mat=matrix(c(1,1,2,2,3,3,4,4,4,5,5,5), nrow=2, byrow=TRUE), heights=c(1,.4))
for(i in 1:3){
with(replist.list[[i]],{
...
with(index2, points(years[[1]], y=rep(3, len=length(years[[1]])), cex=cex.fishery))
...
points(x=25:100, y=rep(4, len=76), cex=cex.fishery)
axis(1, at=seq(25,100, by=25), col=col.border, mgp=par()$mgp, tck=par()$tck)
text(x=90, y=c(1.5,2.5,3.25,4.25), labels=c("Lengths", "Ages", "Index", "Catches"))
box(col=col.border)
## add data poor
...
with(index5, points(years[[1]], y=rep(3, len=length(years[[1]])), cex=cex.fishery))
